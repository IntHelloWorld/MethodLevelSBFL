...
def extract_classes(proj_name, bug_id, test_class, repo_dir, agent_jar, max_num=30):
    ...
    loaded_classes = []
    covered_classes = []
    test_class_name = test_class[0].split("::")[0]
    print(f"[solving {proj_name}-{bug_id}-{test_class_name}]")
    for test_name in test_class:
    ...
    if len(covered_classes) > 0:
        if len(test_class) > 1:
            print(f"    <classes intersection for test suite {proj_name}-{bug_id}-{test_class_name}>")
            ...
            print(f"    <test suite with single failed test {proj_name}-{bug_id}-{test_class_name}>")
            extracted_classes = covered_classes[0][:max_num]
            extracted_classes = [c.class_name for c in extracted_classes]
    else:
        extracted_classes = []
    
    p = os.popen("git clean -df").read()
    os.chdir(cwd)
    cmd = "rm -rf tmp"
    os.system(cmd)
