...
def covered_classes_prompt(version, project, bugID, test_class, model_type, n_classes=50, class_doc_tokens=100, basement="None"):
    """
    ...
    """
    classes_dict = {}
    _, _, extracted_classes = extract_classes(version, project, bugID, test_class, max_num=n_classes)
    if basement == "Grace":
...
def get_top1_prompt(test_classs, buggy_methods):
    test_classs_prompt = ""
    prompt = ""
    for i, test_class in enumerate(test_classs):
        prompt += f"{i+1}) Failed Test Suite: \"{test_class}\"\n"
        test_classs_prompt += f"{i+1}) {test_class}\n"
        cause = ""
        methods = []
        for method in buggy_methods:
            if method["test_class"] == test_class:
        ...
        prompt += "\"\n\n"
    return test_classs_prompt, prompt
